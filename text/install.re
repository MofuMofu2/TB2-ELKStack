...
#@# 上図について、LogstashからElasticsearchの矢印にIndexing、KibanaからElasticsearchの矢印にSearchingと追記してもよいのでは？
...
#@# 上図について、「データ連携の流れの」図と違う箱と矢印を使った方が良い気がしました。同じだとデータ連携の話と混同しそうなので。
...
#@# serviceコマンドはLinux6系までの話であることが分かるようにした方が良いのでは。7系はsystemctlコマンドなため。
#@# [serviceコマンドが自動的にダウンロードされます。]の「ダウンロード：という表現が気になる。「設定されます」が良いのでは。
...
#@# Dockerコンテナのイメージで提供されているのは、Elastic CloudがマルチテナントをDockerで実現しているからなんです
...
#@# 上記の「6」はどこを指しているのか、もう少し明確に記載した方が良いのでは
...
#@# 「ディレクトリ構成は一例です」という表現に違和感あり。「本ディレクトリ構成はあくまで一例です」が良いのでは？
...
#@# 上記ターミナル出力内容について、先頭2行の意図が理解出来なかった。ただのパス指定ミス？
...
#@# 上記変更箇所の説明について、どこかymlファイル内で追記した箇所なのか分かると親切かな。
