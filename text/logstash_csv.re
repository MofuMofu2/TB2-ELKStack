...
記録された履歴はCSV（SJIS/UTF-8）やXMLでダウンロードできます。
...
=== 取り込んだログを標準出力する
outputに@<b>{stdout}を指定すると、コンソール上に標準出力できます。
...
Logstashの起動は先ほどと同じように@<tt>{/bin}配下にある起動スクリプトから行います。
...
@<tt>{tags}を利用すると、if文などを用いて取り込んだデータに対する固定の処理を指定できます。
Kibanaのグラフを作成する際に@<tt>{tags}を指定すると、1つの情報のまとまりごとに
データを分析するグラフを作ることができるようになります。
...
必須項目はありませんが、Elasticsearchのホスト名を指定しないと@<tt>{127.0.0.1}にアクセスします。
...
field内の情報を丸ごと削除したい場合、@<b>{remove_field}オプションを利用します。
...
また、Logstashは世界標準時間（UTC）を利用するので、日本時間からマイナス9時間されてしまいます。
